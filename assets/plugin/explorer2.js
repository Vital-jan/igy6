...
// =====================================================================
  ...
  // Пример: setSlider(4, 'assets/img/slider/', 3, 0.5, 'slider');
  // цвет стрелок и кружочков - по фоновому цвету родительского контейнера
  // цвет текста подписи и активній кружочек - по цвету текста родительского контейнера
  // --------------------------------------------
  // max - к-во изображений
  // path - папка с изображениями для слайдера (имена изображений должны быть img1, img2, img...)
  // delay - время показа изображения в секундах
  // fade - коэффициент от delay (от 0 до 1), время набора 100% непрозрачности при смене кадра
  ...
  // prefix - префикс имени файла изображения (по умолчанию img)
  // ----------------------------------------------------

  let slider = document.querySelector(`#${sliderId}`);
  if (slider == null) return;
  let color = getComputedStyle(slider).color; // цвет текста родителя
  let bgColor = getComputedStyle(slider).backgroundColor; // цвет фона родителя
  ...
  if (arrowsVisible) {
    for (n = 0; n < 2; n++) {
      ...
      arrow[n].style.borderLeftColor = bgColor;
  ...
  function refresh(arrowVisible){
    ...
    if (prevCounter != 0) {
      ...
      if (elem) elem.style.backgroundColor = bgColor;
    }
...
// ==========================================================================================================================
  ...
  // на сенсорних екранах скроллінг відбувається по click, на desktop - по наведенню
    ...
    // duration - тривалість скролінгу в ms на step px (тільки для сенсорних екранів)
...
//----------------------------------------------------------------------------------------------------------------------------------
  ...
  window.onresize = ()=> { // оновлення переліку елементів після зміни розміру екрану
    if (document.body.getBoundingClientRect().width != screenWidth) {
      productRefresh();
    }
  }
